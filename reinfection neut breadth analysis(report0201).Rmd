...
```{r include=FALSE}
pdf('breadth_modelchecking.pdf', width = 20, height = 10)
...
```{r include=FALSE}
pdf('breadth_prediction.pdf', width = 30, height = 20)
...
- Now we interpret the coefficients of the poisson regression. The expected breadth at the first day of infection, theoretically, is $3.07$. We refer this as start point. For each more time infected, the start point will be $1.25$ times than the previous one. This is number effect. Then we look at how the breadth changes as time pass. For each 100 days, the expected breadth will be $1.10$ times than the previous one. We call this time effect. With these coefficients, we can estimate the breath of the patients in group 1. For example, a patient is infected for the second time. On 900 days of infection, the expected neutralization breadth is $3.07 \times 1.25^{(2-1)} \times 1.10^{900/100} = 9.05$.
...
```{r include=FALSE}
pdf('breadth_prediction_divergence_all.pdf', width = 30, height = 20)
...
```{r include=FALSE}
pdf('breadth_prediction_divergence_clear.pdf', width = 30, height = 20)
plot_grid(div.poi.p, div.poi.p1, div.poi.p2, div.poi.p3, div.poi.p4, div.poi.p5, ncol = 3, labels ='AUTO')
dev.off()
...
```{r include=FALSE}
pdf('potency_modelchecking.pdf', width = 10, height = 10)
...
```
```{r include=FALSE}
pdf('potency_prediction.pdf', width = 30, height = 20)
...
var.cov = summary(neut_potency_lmfit_null)$cov
beta = neut_potency_lmfit_null$coefficients
...
|:-----------------------------------:|:-------------------------------:|
|             start point             |    26.9883(26.9392, 27.0373)    |
|            number effect            |     6.1334(6.1016, 6.1651)      |
|             time effect             |     3.3403(3.3394, 3.3413)      |

- Now we interpret the coefficients of the linear regression. The expected potency at the first day of the first infection, theoretically, is $26.99%$. We refer this as start point. For each more time infected, the start point will be $6.13%$ greater than the previous one. This is number effect. Then we look at how the potency changes as time pass. For each more 100 days, the expected potency will be $3.34%$ greater. We call this time effect. With these coefficients, we can estimate the potency of the patients . For example, a patient is infected for the second time. On 900 days of infection, the expected neutralization potency is $26.99% + 6.13% \times (2-1) + 3.34% \times (900/100)} = 63.18%$.
...
```
```{r include=FALSE}
pdf('potency_prediction_divergence_all.pdf', width = 30, height = 20)
...
```{r include=FALSE}
pdf('potency_prediction_divergence_clear.pdf', width = 30, height = 20)
plot_grid(po.div.p, po.div.p1, po.div.p2, po.div.p3, po.div.p4, po.div.p5, ncol = 3, labels ='AUTO')
dev.off()
```
